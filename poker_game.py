...
# --- Funzioni Texas Hold'em ---
def is_straight(values):
    vals = sorted(set(values), reverse=True)
    if 14 in vals: vals.append(1)  # A low straight
...
class Game:
    ...
    def start_hand(self)->Tuple[bool,str]:
        ...
        # Distribuzione carte
        for _ in range(2):
            for p in self.players: p.hole.append(self.deck.draw(1)[0])
        # Blinds
        self.dealer_idx=self.dealer_idx%len(self.players)
        sb_idx=(self.dealer_idx+1)%len(self.players)
        bb_idx=(self.dealer_idx+2)%len(self.players)
        sb_player=self.players[sb_idx]; bb_player=self.players[bb_idx]
        sb_amt=min(self.sb,sb_player.chips); bb_amt=min(self.bb,bb_player.chips)
        sb_player.chips-=sb_amt; sb_player.current_bet=sb_amt; sb_player.all_in=sb_player.chips==0
        bb_player.chips-=bb_amt; bb_player.current_bet=bb_amt; bb_player.all_in=bb_player.chips==0
    ...
    def player_action(self,player_id,action,amount=0)->Tuple[bool,str]:
        ...
        if action=="fold":
            ...
            if nxt is None: self.advance_stage()
            else: self.turn_idx=nxt
            return True,"Fold"
        if action=="check":
        ...
        if action=="raise":
            ...
            if amount<min_raise and amount<p.chips: return False,f"Il raise minimo è {min_raise}"
            total_put=to_call+amount
            if total_put>=p.chips: total_put=p.chips; p.all_in=True
            p.chips-=total_put
            p.current_bet+=total_put
    ...
    def next_active_idx(self,start_idx):
        n=len(self.players)
        for i in range(1,n+1):
            ...
            if p.in_hand and not p.all_in and p.current_bet<self.current_bet:
                return idx
        if self.current_bet==0: return None
        return None
    ...
    def advance_stage(self):
        ...
        self.last_raiser_idx=-1
        self.last_raise_amount=self.bb
        self.turn_idx=self.next_active_idx(self.dealer_idx) or 0
    ...
    def collect_pots_and_award(self):
        ...
        if len(inplay)==1:
            ...
            amt=self.pot
            winner.chips+=amt
            results.append({"winner_name":winner.name,"amount":amt,"hand":"Unico Rimasto"})
            self.pot=0
            self.started=False
            self.stage="waiting"
            return results
        ...
        for i,(w,hand_type) in enumerate(winners):
            amt=split+(remainder if i==0 else 0)
            w.chips+=amt
