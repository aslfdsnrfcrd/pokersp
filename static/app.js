...
joinBtn.onclick = async () => {
  ...
  let res = await fetch("/api/join", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ room_id, name: my_name })
  });
  let j = await res.json();
  if (j.ok) {
...
startBtn.onclick = async () => {
  ...
  let j = await res.json();
  if (!j.ok) alert("Errore: " + j.error);
  else alert("Mano iniziata");
};
...
function cardToAsciiArt(card) {
  // Numero di righe per ogni carta (usato implicitamente dall'altro metodo)
  const lines = 6;

  if (card === "XX") {
    // Dorso della carta (coperta) — ho rimosso il backtick per evitare possibili problemi
    return [
      " _____ ",
      "|#####|",
      "|#####|",
  ...
  // Ritorno l'ASCII art come array di stringhe HTML (con <span> per il colore)
  return [
...
function cardsToAsciiBlock(cards) {
  ...
  let outputHtml = '<pre style="display:inline-block; margin:0; line-height:1;">'; // Usa <pre> per mantenere la formattazione a spaziatura fissa

  for (let i = 0; i < lines; i++) {
    const line = cardArt.map((art) => art[i]).join("");
    outputHtml += line + "\n";
  }
  outputHtml += "</pre>";
...
function renderState(s) {
  ...
  meDiv.innerHTML = my
    ? `<strong>Tu: ${my.name}</strong> Chips: ${my.chips} Bets: ${my.current_bet}<br>${cardsToAsciiBlock(my.hole)}`
    : "Non sei in stanza";
  ...
  s.players.forEach((p) => {
    ...
    const cardsToShow =
      p.id === player_id || s.stage === "SHOWDOWN" || s.stage === "END"
        ? p.hole
        : p.hole.map(() => "XX");
  ...
  if (s.turn_id === player_id) {
    ...
    fold.innerText = "Fold";
    fold.onclick = () => doAction("fold");

    let check = document.createElement("button");
    check.innerText = "Check";
    check.onclick = () => doAction("check");

    let call = document.createElement("button");
    call.innerText = "Call";
    call.onclick = () => doAction("call");
    ...
    controlsDiv.appendChild(check);
    controlsDiv.appendChild(call);
